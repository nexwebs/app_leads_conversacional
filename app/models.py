...
class Lead(Base):
    ...
    conversaciones = relationship(
        "Conversacion", back_populates="lead", cascade="all, delete-orphan"
    )
    mensajes = relationship(
        "Mensaje", back_populates="lead", cascade="all, delete-orphan"
    )

    __table_args__ = (
        CheckConstraint(
            "origen IN ('web_chat', 'whatsapp', 'api')", name="check_origen"
        ),
        CheckConstraint(
            "estado IN ('nuevo', 'asignado', 'calificado', 'vendido', 'descartado')",
            name="check_estado",
        ),
...
class Producto(Base):
    ...
    paquetes = relationship(
        "Paquete", back_populates="producto", cascade="all, delete-orphan"
    )
...
class ConocimientoRAG(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "tipo IN ('caso_exito', 'objecion', 'faq', 'regulacion', 'guia_producto')",
            name="check_tipo_rag",
        ),
    )
